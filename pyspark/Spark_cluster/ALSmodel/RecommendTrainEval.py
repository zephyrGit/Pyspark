#!/usr/bin/env python
# -*- coding: utf-8 -*-
# @Date    : 2019-08-07 10:37:09
# @Author  : Zephyrus (Zephryus@aliyun.com)
# @Link    : https://github.com/zephyrGit
# @Version : $Id$
...
def SetPath(sc):
    ...
    if sc.master[0:5] == "local":
        ...
        Path = "hdfs://master:9000/user/workspace/"
...
def PrepareData(sc):
    ...
    ratingsRDD = rawRatings.map(
        lambda fields: (fields[0], fields[1], fields[2]))
    #----------------------2. 以随机方式将数据分为3 个分并且返回-------------
    trainData, validationData, testRDD = ratingsRDD.randomSplit([8, 1, 1], seed=0)
...
def computeRMSE(alsmodel, RatingRDD):
    ...
    predictionsAndRatings = predictRDD.map(lambda x: ((x[0], x[1]), x[2])).join(
        RatingRDD.map(lambda x: ((float(x[0]), float(x[1])), float(x[2])))).values()

    return sqrt(predictionsAndRatings
                .map(lambda x: (x[0] - x[1]) ** 2) .reduce(add) / float(n))
...
def CreateSparkContext():
    sparkConf = SparkConf() .setAppName("RecommendTrainEval").set(
